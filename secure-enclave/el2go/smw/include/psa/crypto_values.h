...
#define PSA_ALG_DETERMINISTIC_ECDSA(hash_alg) \
    ((psa_algorithm_t)(PSA_ALG_DETERMINISTIC_ECDSA_BASE | ((hash_alg) & (PSA_ALG_HASH_MASK))))
...
#define PSA_ALG_ECDSA(hash_alg) \
    ((psa_algorithm_t)(PSA_ALG_ECDSA_BASE | ((hash_alg) & (PSA_ALG_HASH_MASK))))
...
#define PSA_ALG_GET_HASH(alg)                      \
    ((alg & PSA_ALG_HASH_MASK) == 0 ? PSA_ALG_NONE \
...
#define PSA_ALG_HKDF(hash_alg) \
    ((psa_algorithm_t)(PSA_ALG_HKDF_BASE | ((hash_alg) & (PSA_ALG_HASH_MASK))))
...
#define PSA_ALG_IS_MAC_TRUNCATED(alg)                                              \
    ({                                                                             \
        typeof(alg) _alg = (alg);                                                  \
        PSA_ALG_IS_MAC(_alg) ? ((_alg & PSA_ALG_MAC_TRUNCATION_MASK) ? 1 : 0) : 0; \
    })
 ...
 * This macro can return either 0 or 1 if @alg is not a supported algorithm identifier.
 */
#define PSA_ALG_IS_RANDOMIZED_ECDSA(alg) (((alg) & ~PSA_ALG_HASH_MASK) == PSA_ALG_ECDSA_BASE)

/**
 * PSA_ALG_IS_STANDALONE_KEY_AGREEMENT() - Whether the specified algorithm is a standalone key
 * agreement algorithm.
 * @alg: An algorithm identifier (value of &typedef psa_algorithm_t).
 *
...
#define PSA_ALG_IS_WILDCARD(alg)                                                            \
    (PSA_ALG_IS_HASH_AND_SIGN(alg)  ? PSA_ALG_GET_HASH(alg) == PSA_ALG_ANY_HASH             \
...
#define PSA_ALG_RSA_OAEP(hash_alg) \
    ((psa_algorithm_t)(PSA_ALG_RSA_OAEP_BASE | ((hash_alg) & (PSA_ALG_HASH_MASK))))
...
#define PSA_ALG_RSA_PKCS1V15_SIGN(hash_alg) \
    ((psa_algorithm_t)(PSA_ALG_RSA_PKCS1V15_SIGN_BASE | ((hash_alg) & (PSA_ALG_HASH_MASK))))
...
#define PSA_ALG_RSA_PSS(hash_alg) \
    ((psa_algorithm_t)(PSA_ALG_RSA_PSS_BASE | ((hash_alg) & (PSA_ALG_HASH_MASK))))
 ...
 * - PSA_KEY_DERIVATION_INPUT_CONTEXT is the **Context**. It is optional; if omitted, **Context** is
...
#define PSA_ALG_TLS12_PRF(hash_alg) \
    ((psa_algorithm_t)(PSA_ALG_TLS12_PRF_BASE | ((hash_alg) & (PSA_ALG_HASH_MASK))))
...
#define PSA_ALG_TLS12_PSK_TO_MS(hash_alg) \
    ((psa_algorithm_t)(PSA_ALG_TLS12_PSK_TO_MS_BASE | ((hash_alg) & (PSA_ALG_HASH_MASK))))
 ...
 *
 * Unspecified if @mac_alg is not a supported MAC algorithm or if @mac_length is too small or too
 *large for the specified MAC algorithm.
 */
 ...
 * @lifetime: The lifetime value to query (value of &typedef psa_key_lifetime_t).
 */
#define PSA_KEY_LIFETIME_GET_PERSISTENCE(lifetime) ((psa_key_persistence_t)(lifetime & 0xFF))
 ...
 */
#define PSA_KEY_TYPE_DH_GET_FAMILY(type) ((psa_dh_family_t)((type) & (PSA_KEY_TYPE_DH_GROUP_MASK)))
 ...
 *         be used.
 */
#define PSA_KEY_TYPE_DH_KEY_PAIR(group) ((psa_key_type_t)(PSA_KEY_TYPE_DH_KEY_PAIR_BASE | (group)))
 ...
 * @type: A key type (value of &typedef psa_key_type_t).
 */
#define PSA_KEY_TYPE_IS_RSA_PUBLIC_KEY(type) ((type) == PSA_KEY_TYPE_RSA_PUBLIC_KEY)
