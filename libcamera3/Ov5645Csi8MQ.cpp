...
Ov5645Csi8MQ::Ov5645Csi8MQ(int32_t id, int32_t facing, int32_t orientation, char* path)
    : Camera(id, facing, orientation, path)
{
    mVideoStream = new OvStream(this);
}

Ov5645Csi8MQ::~Ov5645Csi8MQ()
{
}

int Ov5645Csi8MQ::OvStream::getCaptureMode(int width, int height)
{
    int index = 0;
    int ret = 0;
    int capturemode = 0;
...
int Ov5645Csi8MQ::getFps(int width, int height, int /*defValue*/)
{
    int fps = 0;
...
PixelFormat Ov5645Csi8MQ::getPreviewPixelFormat()
{
    ALOGI("%s", __func__);
    return HAL_PIXEL_FORMAT_YCbCr_422_I;
}

status_t Ov5645Csi8MQ::initSensorStaticData()
{
...
int32_t Ov5645Csi8MQ::OvStream::onDeviceConfigureLocked()
{
    ALOGI("%s", __func__);
    int32_t ret = 0;
    if (mDev <= 0) {
    ...
    if(strstr(deviceName, "pico_imx8m")) {
