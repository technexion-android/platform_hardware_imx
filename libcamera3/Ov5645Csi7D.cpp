...
Ov5645Csi7D::Ov5645Csi7D(int32_t id,
                         int32_t facing,
                         int32_t orientation,
                         char* path)
...
Ov5645Csi7D::~Ov5645Csi7D() {}

PixelFormat Ov5645Csi7D::getPreviewPixelFormat()
{
    return HAL_PIXEL_FORMAT_RGBA_8888;
}

int Ov5645Csi7D::getCaptureMode(int width, int height)
{
    int capturemode = -1;
...
status_t Ov5645Csi7D::initSensorStaticData()
{
    ...
    while (ret == 0) {
        ...
        if (usemx6s > 0) {
            ...
            if ((vid_frmsize.discrete.width > 1280) ||
                ...
                // Max fps for ov5645 csi xga cannot reach to 30fps
                vid_frmval.discrete.denominator = 15;
                vid_frmval.discrete.numerator = 1;
            } else {
    ...
    ALOGI("ov5645Csi, mFocalLength:%f, mPhysicalWidth:%f, mPhysicalHeight %f, mMaxJpegSize %d",
                mFocalLength,
                mPhysicalWidth,
                mPhysicalHeight,
                mMaxJpegSize);
...
// configure device.
int32_t Ov5645Csi7D::OvStream::onDeviceConfigureLocked()
{
    ALOGI("%s", __func__);
    if (mDev <= 0) {
