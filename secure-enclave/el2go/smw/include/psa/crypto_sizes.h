...
#define PSA_AEAD_DECRYPT_OUTPUT_SIZE(key_type, alg, ciphertext_length)                 \
    ((PSA_ALG_IS_AEAD(alg) != 0 && (ciphertext_length) > PSA_ALG_AEAD_TAG_LENGTH(alg)) \
...
#define PSA_AEAD_ENCRYPT_OUTPUT_SIZE(key_type, alg, plaintext_length) \
    (PSA_ALG_IS_AEAD(alg) ? ((plaintext_length) + PSA_ALG_AEAD_TAG_LENGTH(alg)) : (0u))
...
#define PSA_AEAD_NONCE_LENGTH(key_type, alg)                                                       \
    (PSA_ALG_IS_AEAD(alg)                                                                          \
             ? ((PSA_ALG_AEAD_WITH_DEFAULT_LENGTH_TAG(alg) == PSA_ALG_CCM)           ? (13u)       \
                        : (PSA_ALG_AEAD_WITH_DEFAULT_LENGTH_TAG(alg) == PSA_ALG_GCM) ? (12u)       \
 ...
 */
#define PSA_AEAD_TAG_LENGTH(key_type, key_bits, alg) PSA_ALG_AEAD_TAG_LENGTH(alg)
...
size_t psa_cipher_encrypt_output_size(psa_key_type_t key_type, psa_algorithm_t alg,
                                      size_t input_length);
...
#define PSA_EXPORT_KEY_PAIR_MAX_SIZE                                           \
    PSA_MAX(PSA_KEY_EXPORT_RSA_KEY_PAIR_MAX_SIZE(PSA_VENDOR_RSA_MAX_KEY_BITS), \
...
#define PSA_EXPORT_PUBLIC_KEY_MAX_SIZE                                           \
    PSA_MAX(PSA_KEY_EXPORT_RSA_PUBLIC_KEY_MAX_SIZE(PSA_VENDOR_RSA_MAX_KEY_BITS), \
...
#define PSA_HASH_SUSPEND_HASH_STATE_FIELD_LENGTH(alg)                               \
    ({                                                                              \
        typeof(alg) _alg = (alg);                                                   \
        ((_alg == PSA_ALG_MD2)                                        ? 64          \
                 : _alg == PSA_ALG_MD4 || _alg == PSA_ALG_MD5         ? 16          \
                 : _alg == PSA_ALG_RIPEMD160 || _alg == PSA_ALG_SHA_1 ? 20          \
                 : _alg == PSA_ALG_SHA_224 || _alg == PSA_ALG_SHA_256 ? 32          \
                 : _alg == PSA_ALG_SHA_512 || _alg == PSA_ALG_SHA_384 ||            \
                         _alg == PSA_ALG_SHA_512_224 || _alg == PSA_ALG_SHA_512_256 \
                 ? 64                                                               \
                 : 0);                                                              \
    })
...
#define PSA_HASH_SUSPEND_INPUT_LENGTH_FIELD_LENGTH(alg)                                       \
    ({                                                                                        \
        typeof(alg) _alg = (alg);                                                             \
        (_alg == PSA_ALG_MD2 ? 1                                                              \
                 : _alg == PSA_ALG_MD4 || _alg == PSA_ALG_MD5 || _alg == PSA_ALG_RIPEMD160 || \
                         _alg == PSA_ALG_SHA_1 || _alg == PSA_ALG_SHA_224 ||                  \
...
#define PSA_HASH_SUSPEND_OUTPUT_SIZE(alg)                                                   \
    ({                                                                                      \
        ...
        (PSA_HASH_SUSPEND_ALGORITHM_FIELD_LENGTH +                                          \
         ...
         PSA_HASH_SUSPEND_HASH_STATE_FIELD_LENGTH(_alg) + PSA_HASH_BLOCK_LENGTH(_alg) - 1); \
    })
...
#define PSA_MAC_TRUNCATED_LENGTH(alg)                                               \
    ({                                                                              \
        ...
        PSA_ALG_IS_MAC_TRUNCATED(_alg)                                              \
                ? (_alg & PSA_ALG_MAC_TRUNCATION_MASK) >> PSA_MAC_TRUNCATION_OFFSET \
                : 0;                                                                \
    })
...
#define PSA_HMAC_LENGTH(alg)                                                                     \
    ({                                                                                           \
        typeof(alg) _alg = (alg);                                                                \
        PSA_ALG_IS_HMAC(_alg) ? (PSA_MAC_TRUNCATED_LENGTH(_alg) ? PSA_MAC_TRUNCATED_LENGTH(_alg) \
                                                                : PSA_MAC_MAX_SIZE)              \
                              : 0;                                                               \
    })
...
#define PSA_BLOCK_CIPHER_MAC_LENGTH(alg)                                             \
    ({                                                                               \
        ...
        PSA_ALG_IS_BLOCK_CIPHER_MAC(_alg)                                            \
                ? (PSA_MAC_TRUNCATED_LENGTH(_alg) ? PSA_MAC_TRUNCATED_LENGTH(_alg)   \
...
#define PSA_MAC_LENGTH(key_type, key_bits, alg) \
    (PSA_ALG_IS_HMAC(alg) ? PSA_HMAC_LENGTH(alg) : PSA_BLOCK_CIPHER_MAC_LENGTH(alg))
 ...
 */
#define PSA_MAC_MAX_SIZE PSA_MAX(PSA_HASH_MAX_SIZE, PSA_BLOCK_CIPHER_BLOCK_MAX_SIZE)
...
#define PSA_ECC_SIGNATURE_MAX_SIZE PSA_ECC_SIGNATURE_SIZE(PSA_VENDOR_ECC_MAX_CURVE_BITS)
#define PSA_RSA_SIGNATURE_MAX_SIZE PSA_BITS_TO_BYTES(PSA_VENDOR_MAX_RSA_KEY_BITS)
 ...
 */
#define PSA_SIGNATURE_MAX_SIZE PSA_MAX(PSA_ECC_SIGNATURE_MAX_SIZE, PSA_RSA_SIGNATURE_MAX_SIZE)
...
#define PSA_SIGN_OUTPUT_SIZE(key_type, key_bits, alg)                                   \
    (PSA_KEY_TYPE_IS_RSA(key_type)           ? ((void)alg, PSA_BITS_TO_BYTES(key_bits)) \
             : PSA_KEY_TYPE_IS_ECC(key_type) ? PSA_ECC_SIGNATURE_SIZE(key_bits)         \
                                             : ((void)alg, 0))
