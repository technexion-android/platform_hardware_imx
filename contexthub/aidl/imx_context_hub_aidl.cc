...
ScopedAStatus ContextHub::enableNanoapp(int32_t /* contextHubId */,
                                        int64_t appId,
                                        int32_t /* transactionId */) {
  ALOGW("Attempted to enable app ID 0x%016" PRIx64 ", but not supported", appId);
  return ScopedAStatus::fromExceptionCode(EX_UNSUPPORTED_OPERATION);
}
...
ScopedAStatus ContextHub::onHostEndpointConnected(
  ...
  switch (in_info.type) {
    ...
    case HostEndpointInfo::Type::FRAMEWORK:
      ...
      break;
    default:
      ALOGE("Unsupported host endpoint type %" PRIu32, type);
      return ScopedAStatus::fromExceptionCode(EX_ILLEGAL_ARGUMENT);
  }
...
ScopedAStatus ContextHub::disableTestMode() {
  ...
  if (!mIsTestModeEnabled) {
     ...
     * multiple at a time.
     */
    ALOGE("There exists a pending load transaction. Cannot disable test mode.");
    ...
    if (!loadNanoappsInternal(kDefaultHubId, nanoappsToLoad)) {
      ALOGE("Unable to load all preloaded, non-system nanoapps.");
    }
  ...
  if (success) {
    ...
    ALOGI("Successfully disabled test mode.");
    return ScopedAStatus::ok();
  } else {
    return ScopedAStatus::fromExceptionCode(EX_SERVICE_SPECIFIC);
  }
...
bool ContextHub::loadNanoappsInternal(
  ...
  for (const NanoappBinary &nanoappToLoad : nanoappBinaryList) {
    ALOGI("Loading nanoapp with ID: 0x%016" PRIx64, nanoappToLoad.nanoappId);

    bool success = false;
    if (!loadNanoappInternal(nanoappToLoad,
                             *mSynchronousLoadUnloadTransactionId)) {
      ALOGE("Failed to request loading nanoapp with ID 0x%" PRIx64,
      ...
      if (mSynchronousLoadUnloadSuccess.has_value() &&
        ...
        ALOGI("Successfully loaded nanoapp with ID: 0x%016" PRIx64,
             nanoappToLoad.nanoappId);
        success = true;
      }
    }
    ...
    if (!success) {
      ALOGE("Failed to load nanoapp with ID 0x%" PRIx64,
           nanoappToLoad.nanoappId);
    }
    ++(*mSynchronousLoadUnloadTransactionId);
  }
  ...
  for (auto &preloadedNanoapp : preloadedNanoapps) {
    ...
    if (!isSystemNanoapp) {
      ...
      if (!readFileContents(nanoappFile.c_str(), nanoappBuffer)) {
        ALOGE("Cannot read header file: %s", nanoappFile.c_str());
      } else {
