...
Ov5645Mipi::Ov5645Mipi(int32_t id, int32_t facing, int32_t orientation, char* path)
    : Camera(id, facing, orientation, path)
{
    mVideoStream = new OvStream(this);
}

Ov5645Mipi::~Ov5645Mipi()
{
}

status_t Ov5645Mipi::initSensorStaticData()
{
    ALOGV("%s", __func__);
    ...
    ALOGI("ov5645Mipi, mFocalLength:%f, mPhysicalWidth:%f, mPhysicalHeight %f",
...
// configure device.
int32_t Ov5645Mipi::OvStream::onDeviceConfigureLocked()
{
    ALOGI("%s", __func__);
    int32_t ret = 0;
    if (mDev <= 0) {
